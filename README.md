...
> **보조 참조 (HotpotQA / 2WikiMHQA):** HotpotQA — A0(0.730), A5(0.669, -0.061), A8(0.786, +0.056). 2WikiMHQA — A0(0.790), A5(0.754, -0.036), A8(0.824, +0.034). 방향성은 AgXQA·MuSiQue와 일치.
...
> Ablation이 **무엇**이 기여하는지를 묻는다면, Case Study는 **왜** 그런 결과가 나왔는지를 질의 단위로 해석합니다.

---

#### C1. Graph Harmful Case — Pull-down 방어 (AgXQA, 1-hop)

**Query (QID 1185383):** *"What are scale dependent with regard to irrigation?"*
**Gold Answer:** Irrigation systems
**Ground Truth Document:** `bc9d2fc8ccb580ea` — *"Options for sharing irrigation equipment. Irrigation systems are very scale dependent. Sharing the irrigation expense by jointly investing with a …"*

| Method | nDCG@10 | 설명 |
|--------|:-------:|------|
| RRF (D+S) | **1.000** | Dense+Sparse가 정답 문서를 Top-1에 정확히 배치 |
| RRF (D+S+G, 1:1:1) | **0.000** | Graph 채널이 "irrigation" 엔티티로 무관한 문서를 대량 투입, 정답이 Top-10 밖으로 밀려남 |
| **ours\_structural (DAT)** | **1.000** | DAT가 Graph를 억제하여 D+S 랭킹을 보존 |
...
**분석:** AgXQA는 1-hop 정의형 질의가 대다수이며, Graph 채널의 엔티티 매칭이 노이즈로 작동합니다. DAT는 tune 데이터에서 이 패턴을 학습하여 **α\_graph=0.00**으로 자동 설정했고, 이 질의에서 naive 3ch가 만든 **nDCG 0 → 1.000 완전 복구**를 달성했습니다.

---

#### C2. Graph Helpful Case — 조건부 이득 (MuSiQue, 2-hop)

**Query (QID 2hop\_\_82713\_84616):** *"What did the actress in My Fair Lady win a Tony for?"*
**Gold Answer:** for Best Performance by a Leading Actress in a Play in Ondine
**Ground Truth Documents:**
- `902ab8870c02e22f` — *"My Fair Lady (film): The film stars Audrey Hepburn as Eliza Doolittle and Rex Harrison as Henry Higgins …"*
- `58d1ac8bd886b01c` — *"List of awards and honours received by Audrey Hepburn: Audrey Hepburn received numerous awards and honors during her career …"*

| Method | nDCG@10 | 설명 |
|--------|:-------:|------|
| RRF (D+S) | **0.544** | Doc1(My Fair Lady)만 검색. Doc2(수상 이력)는 "My Fair Lady"와 의미적 거리가 멀어 놓침 |
| RRF (D+S+G, 1:1:1) | **0.442** | Graph로 Doc2 일부 발견했으나, 무관한 엔티티 매칭 노이즈가 함께 유입되어 오히려 하락 |
| **ours\_structural (DAT)** | **1.000** | Graph 채널을 **선택적 활성화**하여 `Audrey Hepburn` → `awards` 엔티티 체인으로 Doc2 복원 |

**DAT 프로파일 (MuSiQue, 해당 세그먼트):**

| 채널 | 세그먼트 가중치 | 해석 |
|:----:|:--------------:|------|
| Dense | **0.80** | 여전히 의미 매칭 주력 |
| Sparse | **0.00** | 키워드 매칭 불필요 판단 |
| Graph | **0.20** | **선택적 활용** — 엔티티 체인 탐색용 |

**분석:** 이 질의는 "My Fair Lady의 여배우"(Doc1) → "그 배우의 수상 이력"(Doc2)의 2-hop 추론이 필요합니다. Dense만으로는 Doc2를 의미적으로 연결하지 못했지만, Graph 채널은 `Audrey Hepburn` 엔티티를 통해 Doc2를 발견했습니다. DAT는 medium-length·no-entity 세그먼트에 **α\_graph=0.20**을 배분하여, Graph의 기여를 살리면서도 노이즈를 제한했습니다.

---

> **C1과 C2의 대비가 DAT의 핵심 가치를 보여줍니다:** 같은 시스템이 AgXQA에서는 α\_graph=0.00으로 Graph를 완전 억제하고, MuSiQue에서는 α\_graph=0.20으로 선택적 활용합니다. 이 **질의 유형별 적응성**이 고정 가중치 방식과의 결정적 차이입니다.
