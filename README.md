...
| **C3** | **엔드투엔드 엣지 배포** | 멀티모달 문서 파싱 → 지식 그래프·벡터 인덱스 구축 → 경량 LLM 추론 → 자동화 품질 평가를 하나의 재현 가능한 파이프라인으로 통합. Jetson AGX Orin 기준 reference-platform 배포와 bounded device-level 실측을 제공 |
...
| **RQ3** | 전체 시스템이 엣지 reference platform에서 배포 가능하며, serving-time adaptive weighting 비용을 줄이는가? | Jetson CUDA 실측: runtime 비교 (Table 6) + QACT deployment memory footprint (Table 7) |
...
> Table 1–5는 **실험 결과 확정**, RQ3는 Jetson AGX Orin + CUDA `llama.cpp` 실측값으로 반영했다.
...
> **RQ3 해석 원칙:** runtime 비교는 `LLM-based runtime weighting` 대 `QACT lookup weighting`으로 읽고, 메모리 표는 `QACT` 자체의 deployment footprint로 읽는다.

### Table 6. Edge Runtime Comparison (RQ3)

| Weighting Strategy | E2E p50 (ms) | E2E p95 (ms) | Retrieval Mean (ms) | Generation p50 (ms) | QPS |
|--------------------|:------------:|:------------:|:-------------------:|:-------------------:|:---:|
| `LLM-based runtime weighting` | 11,502.89 | 11,585.87 | 3,821.75 | 7,636.51 | 0.087 |
| `QACT` lookup weighting | 8,545.77 | 8,682.35 | 957.71 | 7,534.97 | 0.117 |

### Table 7. QACT Deployment Memory Footprint (RQ3)

| Metric | QACT |
|--------|:----:|
| `steady-state RSS, total (MB)` | 5,402.82 |
| `llama RSS (MB)` | 4,025.71 |
| `end-to-end active RSS peak (MB)` | 5,407.54 |
| `weighting-stage add. host mem., peak (MB)` | 8.89 |
| `end-to-end add. host mem., peak (MB)` | 23.71 |

**RQ3 핵심 해석:**
- Jetson AGX Orin reference platform에서 full serving stack이 실제로 동작했다.
- `QACT`는 `LLM-based runtime weighting` 대비 retrieval mean latency를 크게 줄이면서 generation latency는 거의 유지했다.
- `QACT`의 steady-state resident footprint는 약 5.4 GB이고, query 처리 중에도 active RSS peak가 baseline에 가깝게 유지된다.
