...
| 파라미터 | 값 | 설명 |
|----------|-----|------|
| `use_rrf` | true | RRF 기반 융합 활성화 |
| `rrf_k` | 60 | RRF 파라미터 |
| `use_dat` | true | 동적 가중치 조정 활성화 |
| `use_ontology` | true | 온톨로지 매칭 활성화 |
| `use_pathrag` | false | PathRAG 활성화 (선택적) |
...
2. **멀티 채널**: 수치/단위 정보(EC, pH)의 정확한 매칭을 위한 Sparse 채널과 인과관계 추론을 위한 PathRAG 채널 유지
