...
┃                 ▼                  ┃                 ▼                     ┃  │               │ • 220 QA pairs       │      │   ┃
┃  ┌──────────────────────────────┐  ┃  ┌─────────────────────────────────┐  ┃  │               │ • 6 categories       │      │   ┃
┃  │ ③ KNOWLEDGE STORE            │  ┃  │ ⑥ RESULT PREPARATION           │  ┃  │               │ • ROUGE-L 0.93       │      │   ┃
┃  │ ─────────────────────────    │  ┃  │ ─────────────────────────────  │  ┃  │               │ • Groundedness 0.52  │      │   ┃
┃  │ ┌──────────────────────────┐ │  ┃  │ Top-k selection and formatting │  ┃  │               └─────────────────────┘      │   ┃
┃  │ │ dense.faiss (mmap)       │ │  ┃  │ for LLM context                │  ┃  └─────────────────────────────────────────────┘   ┃
┃  │ │ • FAISS IndexFlatIP      │ │  ┃  │                                │  ┃                                                     ┃
┃  │ │ • MiniLM / Qwen3-Emb     │ │  ┃  │                                │  ┃  ┌─────────────────────────────────────────────┐   ┃
┃  │ ├──────────────────────────┤ │  ┃  │                                │  ┃  │ VERIFICATION                                │   ┃
┃  │ │ sparse.pkl               │ │  ┃  │                                │  ┃  │ ─────────────────────────────────────       │   ┃
...
┃  │ │ • Built from doc metadata│ │  ┃  │          Final Top-k (4)       │  ┃  │   (range/unit validation)                   │   ┃
┃  │ ├──────────────────────────┤ │  ┃  │               ▼                │  ┃  │ • Prompt Constraints                        │   ┃
┃  │ │ ontology.json            │ │  ┃  └──────────────────────────────┬┘  ┃  └─────────────────────────────────────────────┘   ┃
┃  │ │ • 6 types: crop, env,    │ │  ┃                                 │   ┃                                                     ┃
...
┃  │ • Bilingual EN-KO            │  ┃  │ │ RAM < 0.8GB  → none (skip) │ │  ┃  │ • w/o Reranking (no reranking step)        │   ┃
...
digraph ERA_SmartFarm_RAG {
    ...
    subgraph cluster_phase1 {
        label="🔵 PHASE 1: Data & Index Build\n(Offline, One-time)";
        style=filled;
        fillcolor="#E3F2FD";
        color="#1976D2";
        penwidth=2;
    ...
    subgraph cluster_phase2 {
        label="🔵 PHASE 2: Runtime Inference\n(Online, Per-query)";
        style=filled;
        fillcolor="#E3F2FD";
        color="#1976D2";
        penwidth=2;
        
        qa [label="④ Query Analysis\n─────────────\nOntologyMatcher.match()\nDynamic Alpha Tuning\n수치→α_s+0.2\n병해→α_p~0.3" shape=box];
        hr [label="⑤ HybridDAT Retrieval\n─────────────\nDense (FAISS) | Sparse (TF-IDF)\nPathRAG (BFS 2-hop)\nScore Fusion" shape=box];
        cs [label="⑥ Result Preparation\n─────────────\nTop-k selection\nFormat for LLM" shape=box];
        rr [label="⑥.5 Reranking\n─────────────\nRAM<0.8→none\n0.8-1.5→LLM-lite\n≥1.5→BGE" shape=box style=dashed];
...
| ✅ **시스템 아키텍처 구조** | 포함 | Phase 2 (④⑤⑥⑦) | HybridDAT, Reranking, LLM |
...
```mermaid
flowchart LR
    subgraph Phase1["🔵 PHASE 1: Data & Index Build<br/>(Offline, One-time)"]
        direction TB
        DC["① Data Collection<br/>───────────<br/>Web Crawl (Wiki, Brit.)<br/>농촌진흥청 PDF<br/>스마트팜코리아 가이드<br/>EN/KO Bilingual"]
        PP["② Preprocessing<br/>───────────<br/>OCR (EasyOCR)<br/>Sentence-window chunking<br/>Metadata tagging<br/>EN→KO Translation"]
        KS["③ Knowledge Store<br/>───────────<br/>dense.faiss (mmap)<br/>sparse.pkl (TF-IDF)<br/>Causal Graph<br/>ontology.json"]
        STATS["📊 ~100 docs<br/>~400 chunks"]
        DC --> PP --> KS --> STATS
    end
    
    subgraph Phase2["🔵 PHASE 2: Runtime Inference<br/>(Online, Per-query)"]
        direction TB
        QA["④ Query Analysis<br/>───────────<br/>OntologyMatcher<br/>Dynamic Alpha Tuning<br/>수치→α_s+0.2<br/>병해→α_p~0.3"]
        HR["⑤ HybridDAT Retrieval<br/>───────────<br/>Dense|Sparse|PathRAG<br/>Score Fusion"]
        CS["⑥ Result Preparation<br/>───────────<br/>Top-k selection<br/>Format for LLM"]
        RR["⑥.5 Reranking<br/>───────────<br/>RAM-aware selection<br/>BGE/LLM-lite/none"]
        LLM["⑦ LLM Generation<br/>───────────<br/>Qwen3-0.6B Q4_K_M<br/>Fallback Chain"]
        OUT["📤 OUTPUT<br/>{answer, sources,<br/>confidence}"]
        QA --> HR --> CS --> RR --> LLM --> OUT
    end
    
    subgraph EvalLane["🟠 EVALUATION PROTOCOL"]
        direction TB
        ...
        subgraph DatasetGen["Dataset Generation (dataset-pipeline)"]
            ...
            DATASET["📦 QA Dataset<br/>220 pairs<br/>ROUGE-L 0.93"]
            SI --> EI --> RAFT --> JUDGE --> DATASET
        end
        
        VER["Verification<br/>───────────<br/>Source Attribution<br/>Groundedness<br/>Numerical Consistency"]
        ABL["Ablation Study<br/>───────────<br/>Dense-only<br/>Sparse-only<br/>w/o PathRAG<br/>w/o Shaping"]
        BENCH["Benchmark<br/>───────────<br/>vs Dense-only<br/>vs Sparse-only<br/>vs Naive Hybrid"]
    ...
    subgraph Legend["LEGEND"]
        L1["🔵 System Pipeline: Edge Device (8GB RAM)"]
...
**ERA-SmartFarm-RAG의 검색 후처리 전략**:

| 단계 | 기법 | 효과 |
|------|------|------|
| **Dynamic Alpha** | 수치 질의 → Sparse 강화 | 검색 정밀도 향상 |
| **Reranking** | Memory-aware 리랭킹 (선택) | 품질 향상 |
...
│                                                             │
│  Retrieved Results  ──▶  Rerank (Optional)  ──▶  Top-k     │
│       (initial)           BGE / LLM-lite         (k=4)     │
│                                │                            │
│                                ▼                            │
│                      Final Top-k (4)                        │
│                      → LLM Context                          │
...
┃ └────────────────────────┘ └────────────────────────────────────┘   ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃ Layer 3. Result Refinement (검색 후처리)                             ┃
┃ ┌──────────────┐ ┌────────────────┐ ┌────────────────────────────┐  ┃
┃ │ Score Fusion │ │ Top-k Select   │ │ Memory-aware Reranking     │  ┃
┃ │ (α weights)  │ │ (k=4)          │ │ BGE/LLM-lite/none by RAM   │  ┃
┃ └──────────────┘ └────────────────┘ └────────────────────────────┘  ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
...
| **토큰 절감** | SCR (Selective Content Reduction) | **Top-k + Optional Reranking** |
...
- Layer 2의 3-Channel Hybrid Retrieval을 논문 핵심 기여로 하이라이트
...
```
┌──────────────────────────────────────────────────────────────────┐
│              Top-k Selection 효과 (토큰 절감 vs 품질)             │
│                                                                  │
│  Quality                                                         │
│     ▲                                                            │
│     │        ●─────────●                                         │
...
3. **효율적 결과 선택**: Top-k 제한 + 메모리 적응형 Reranking (구현 단순)
