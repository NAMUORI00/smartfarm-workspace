...
## 0. 메인 피겨: 통합 End-to-End 아키텍처 (Figure 1 - 논문 핵심)

> **피드백 반영**: 데이터 수집부터 검증까지 전체 흐름을 **단일 다이어그램**으로 표현

### 0.1 Full Flow Architecture (권장 메인 피겨)

```
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃                           ERA-SmartFarm-RAG: End-to-End Architecture                   ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┳━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃   PHASE 1: 데이터 준비        ┃   PHASE 2: 실시간 추론 (Online)                        ┃
┃   (Offline, 1회)              ┃                                                        ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃                               ┃                                                        ┃
┃  ┌─────────────────────────┐  ┃  ┌─────────────────────────────────────────────────┐   ┃
...
┃  │  ───────────────────    │  ┃  │  ─────────────────────────────────────────────  │   ┃
┃  │  • PDF 매뉴얼           │  ┃  │                                                 │   ┃
┃  │  • 이미지 (스캔)        │  ┃  │   User Query ──► Ontology    ──► Dynamic Alpha  │   ┃
┃  │  • 텍스트 가이드        │  ┃  │   "와사비 고온   Matching       Calculation     │   ┃
┃  │  • 현장 기록            │  ┃  │    관리 방법?"   {crop:와사비   α_d=0.35        │   ┃
┃  └───────────┬─────────────┘  ┃  │                  env:온도}     α_s=0.35        │   ┃
┃              │                ┃  │                               α_p=0.30        │   ┃
┃              ▼                ┃  └──────────────────────┬──────────────────────────┘   ┃
┃  ┌─────────────────────────┐  ┃                         │                              ┃
┃  │     📝 전처리            │  ┃                         ▼                              ┃
┃  │  ───────────────────    │  ┃  ┌─────────────────────────────────────────────────┐   ┃
┃  │  • OCR (EasyOCR)        │  ┃  │  ⚡ 3채널 하이브리드 검색 (HybridDAT)            │   ┃
┃  │  • 시맨틱 청킹          │  ┃  │  ─────────────────────────────────────────────  │   ┃
┃  │  • 수치 정규화          │  ┃  │  ┌─────────┐  ┌──────────┐  ┌────────────────┐  │   ┃
┃  │  • 메타데이터 추출      │  ┃  │  │ Dense   │  │ Sparse   │  │ PathRAG-lite   │  │   ┃
┃  │    - crop: 와사비       │  ┃  │  │ (FAISS) │  │ (TF-IDF) │  │ (Causal Graph) │  │   ┃
┃  │    - category: 환경관리 │  ┃  │  │         │  │          │  │                │  │   ┃
┃  │    - causal_role: cause │  ┃  │  │ α_d×    │  │ α_s×     │  │ α_p×           │  │   ┃
┃  └───────────┬─────────────┘  ┃  │  └────┬────┘  └────┬─────┘  └───────┬────────┘  │   ┃
┃              │                ┃  │       │           │                │            │   ┃
┃              ▼                ┃  │       └───────────┴────────────────┘            │   ┃
┃  ┌─────────────────────────┐  ┃  │                    │                            │   ┃
...
┃  │  ┌───────────────────┐  │  ┃                         │                              ┃
┃  │  │ Dense Index       │  │  ┃                         ▼                              ┃
┃  │  │ dense.faiss       │◄─┼──╋─── mmap load ──────────┐                               ┃
┃  │  │ (mmap)            │  │  ┃                        │                               ┃
┃  │  └───────────────────┘  │  ┃  ┌─────────────────────────────────────────────────┐   ┃
┃  │  ┌───────────────────┐  │  ┃  │  🎯 컨텍스트 압축 (Context Shaping)              │   ┃
┃  │  │ Sparse Index      │  │  ┃  │  ─────────────────────────────────────────────  │   ┃
┃  │  │ sparse.pkl        │◄─┼──╋──┤                                                 │   ┃
┃  │  └───────────────────┘  │  ┃  │   16 docs ──► Crop Filter ──► Semantic Dedup    │   ┃
┃  │  ┌───────────────────┐  │  ┃  │              (+0.5/-0.85)     (θ=0.85)          │   ┃
┃  │  │ Causal Graph      │  │  ┃  │              ≈12 docs         ≈8 docs           │   ┃
┃  │  │ graph.json        │◄─┼──╋──┤                                                 │   ┃
┃  │  │ causes/solved_by  │  │  ┃  │                     │                           │   ┃
┃  │  └───────────────────┘  │  ┃  │                     ▼                           │   ┃
┃  │  ┌───────────────────┐  │  ┃  │           Memory-aware Reranking                │   ┃
┃  │  │ Ontology          │  │  ┃  │           (BGE/LLM-lite/none by RAM)            │   ┃
┃  │  │ 6 concept types   │  │  ┃  │                     │                           │   ┃
┃  │  └───────────────────┘  │  ┃  │                     ▼                           │   ┃
┃  └─────────────────────────┘  ┃  │               Final Top-k (4 docs)              │   ┃
┃                               ┃  └──────────────────────┬──────────────────────────┘   ┃
...
┃                               ┃                                                        ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┻━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃                              EDGE CONSTRAINTS (8GB RAM Target)                         ┃
┃  ┌───────────────────┐  ┌───────────────────┐  ┌───────────────────┐                   ┃
┃  │ LLM: ~2.5GB       │  │ Index: mmap       │  │ Latency: p95<300ms│                   ┃
┃  │ Q4_K_M 양자화     │  │ (lazy load)       │  │ Throughput: 3-8QPS│                   ┃
┃  └───────────────────┘  └───────────────────┘  └───────────────────┘                   ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```

### 0.2 Figure 1 포함 요소 체크리스트 (피드백 요구사항)

| 요구사항 | 반영 여부 | 위치 |
|----------|----------|------|
| ✅ **데이터 컬렉션 및 전처리** | 포함 | Phase 1: 데이터 수집, 전처리, 지식 저장소 구축 |
| ✅ **시스템 아키텍처 구조** | 포함 | Phase 2: 3채널 검색, Context Shaping, LLM |
| ✅ **결과 생성 및 검증** | 포함 | 응답 생성 + 검증 & 출력 (Source Attribution, Hallucination Detection) |
| ✅ **단일 다이어그램** | 충족 | 전체 흐름이 하나의 Figure에 통합 |
| ✅ **엣지 제약 명시** | 포함 | 하단 EDGE CONSTRAINTS 박스 |

### 0.3 Mermaid 버전 (렌더링용)

```mermaid
flowchart TB
    subgraph Phase1["<b>PHASE 1: 데이터 준비 (Offline)</b>"]
        direction TB
        DC["📥 데이터 수집<br/>PDF, 이미지, 텍스트"]
        PP["📝 전처리<br/>OCR, 청킹, 메타데이터"]
        KS["🗄️ 지식 저장소<br/>Dense/Sparse/Graph"]
        DC --> PP --> KS
    end
    
    subgraph Phase2["<b>PHASE 2: 실시간 추론 (Online)</b>"]
        direction TB
        
        subgraph Search["🔍 질의 분석 & 검색"]
            QA["Ontology Matching<br/>Dynamic Alpha"]
            HR["3채널 HybridDAT<br/>Dense+Sparse+PathRAG"]
            QA --> HR
        end
        
        subgraph Context["🎯 컨텍스트 압축"]
            CF["Crop Filter"]
            SD["Semantic Dedup"]
            RR["Reranking"]
            CF --> SD --> RR
        ...
        subgraph Gen["💬 응답 생성"]
            LLM["llama.cpp<br/>Q4_K_M"]
            FB["Fallback<br/>(Cache/Template)"]
            LLM --> FB
        end
        ...
        Search --> Context --> Gen --> Ver
    end
    
    subgraph Output["📤 출력"]
        OUT["Answer + Sources<br/>+ Confidence"]
    end
    
    Phase1 -->|"mmap load"| Phase2
    Phase2 --> Output
    
    subgraph Edge["⚡ Edge Constraints"]
        E1["8GB RAM"]
        E2["p95 < 300ms"]
    end
```
