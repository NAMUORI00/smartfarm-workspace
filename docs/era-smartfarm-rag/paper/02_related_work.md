...
|--------|-----------|-----|--------------|
| **Graph RAG** | LightRAG [4], LinearRAG [6] | 인덱싱(구축) 비용/자원 소모, 엣지 배포 난점 | Tri-Graph 기반 multi-hop 채널 + Dense/Sparse 결합 (Fusion) |
...
본 연구는 기존 Graph RAG 및 하이브리드 검색 연구(LightRAG [4], LinearRAG [6], RRF [12])의 개념을 참고하되, **온프레미스 엣지 배포를 전제로 한 “LLM-free 인덱싱 + 다중 홉 검색” 중심의 Tri-Graph RAG 구조**를 독자적으로 구현한다:
